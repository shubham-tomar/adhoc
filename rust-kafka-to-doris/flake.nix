{
  description = "A kafka to doris data service";
  ...
  outputs = { self, flake-utils, nixpkgs, fenix }:
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in rec {
        # `nix build`
        packages.kafka-to-doris = pkgs.buildRustPackage {
          pname = "kafka-to-doris";
          version = "0.1.0";
        ...
        defaultPackage = packages.kafka-to-doris;

        # `nix run`
        apps.app = flake-utils.lib.mkApp {
          drv = packages.kafka-to-doris;
          name = "app";
        };
